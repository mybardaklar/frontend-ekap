...
// import { useNavigate } from 'next/navigation'; // Next.js uses useRouter from next/navigation
import { useRouter } from 'next/navigation';
import { Card, CardContent, CardHeader, CardFooter, CardDescription, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Check, Info, Shield, CreditCard, ChevronDown, ChevronUp } from "lucide-react";
import { createClient } from "@/utils/supabase/client";
import { toast } from "sonner";
import Link from 'next/link';

// Mock payment handler matching legacy project logic tentatively
// In a real scenario, this would call your payment provider endpoint
const usePayment = () => {
  ...
  const handlePurchase = async (packageCode: string) => {
    ...
    try {
      ...
      // Logic from legacy project: Call 'paytr-payment' function
      const invokeRes = await supabase.functions.invoke('paytr-payment', {
        body: { packageCode }
      });

      if (!invokeRes.error && invokeRes.data?.iframe_url) {
        window.location.href = invokeRes.data.iframe_url;
        return;
      }

      console.warn('⚠️ invoke failed, trying direct fetch fallback', invokeRes.error);

      // 2) Fallback: Direct fetch with full URL
      const { data: sessionData } = await supabase.auth.getSession();
      const jwt = sessionData.session?.access_token;

      const functionUrl = 'https://kodwclnacadhnnmfpvmi.supabase.co/functions/v1/paytr-payment';
      ...
      const resp = await fetch(functionUrl, {
        ...
        headers: {
          ...
          // Supabase Edge Functions often require the anon key in the 'apikey' header
          // and Authorization header to pass the gateway/auth check
          'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '',
          'Authorization': `Bearer ${jwt ?? process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}`,
        },
      ...
      if (!resp.ok) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn('Dev Mode: Payment service failed (likely 401 due to local/prod mismatch). Simulating success.');

          if (resp.status !== 401) {
             console.error('Payment Error Details:', { status: resp.status, body: errorText });
          }

          toast.success("Dev Mode: Ödeme servisi yanıt vermedi, işlem simüle ediliyor.");
          return;
        }

        console.error('Payment Fallback Error:', JSON.stringify({
          status: resp.status,
          statusText: resp.statusText,
          body: errorText
        }, null, 2));

        throw new Error(`Payment service returned ${resp.status}: ${errorText.slice(0, 100)}`);
      }
      ...
      if (data?.iframe_url) {
        ...
        toast.error("Ödeme başlatılamadı: Geçersiz yanıt");
      }
    } catch (error: any) {
      console.error('Payment Error Details:', error);
      toast.error(`Ödeme hatası: ${error.message || 'Bilinmeyen hata'}`);
    } finally {
      setLoading(false);
    }
  };
...
export default function PricingPage() {
  const { handlePurchase, loading } = usePayment();
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {packages.map((pkg) => (
            ...
            >
              ...
              >
                {loading ? 'İşleniyor...' : 'Paketi Satın Al'}
              </Button>
            </div>
          ))}
        </div>
